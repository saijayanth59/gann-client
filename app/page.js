...
export default function Home() {
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center p-4 transition-colors duration-300 ${isDarkMode ? "dark bg-gray-900" : "bg-gray-100"}`}
    >
      <Card className="w-full max-w-2xl bg-white dark:bg-gray-800 transition-colors duration-300">
        ...
        <CardContent>{showUploader ? <CSVUploader /> : <CSVProcessor />}</CardContent>
      </Card>
    </div>
  )
}
